...
import com.hzy.views.rolling.strategy.IStrategy;
import com.hzy.views.rolling.strategy.MultiLineStrategy;
import com.hzy.views.rolling.strategy.SingleLineStrategy;
...
public class VerticalRollingTextView extends View {
    ...
    private static final int[] ATTRS = new int[]{android.R.attr.textSize, android.R.attr.textColor, android.R.attr.ellipsize, android.R.attr.maxLines, android.R.attr.duration};

    private static final String TAG = "VerticalRollingTextView";

    private final static int AUTO_SIZE = -2;

    private VerticalRollingTextAdapter mVerticalRollingTextAdapter;

    //平缓过度Adapter
    private VerticalRollingTextAdapter mTempAdapter;
    ...
    private void parseAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mMinTextSize = arr.getDimensionPixelSize(R.styleable.VerticalRollingTextView_minTextSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 12, getResources().getDisplayMetrics()));
    ...
    protected void onDraw(Canvas canvas) {
        // 绘制文本
        if (mVerticalRollingTextAdapter == null || mVerticalRollingTextAdapter.isEmpty()) {
            return;
        }
        ...
        while (true) {
            ...
            index = index >= mVerticalRollingTextAdapter.getItemCount() ? (index % mVerticalRollingTextAdapter.getItemCount()) : index;
    ...
    private LayoutWithTextSize findLayoutByIndex(int index) {
        ...
        CharSequence cs = mVerticalRollingTextAdapter.getText(index);
        ...
        lt = mStrategy.getLayout(mMinTextSize, autoSizeMaxTextSizeInPx, autoSizeStepGranularityInPx, mTextSize, getWidth(), itemHeight, mPaint, mMaxLines, cs, mTruncateAt);
    ...
    /**
     * 会重置滚动距离胡数据，造成视觉突变，如果想要平滑过渡使用{@link VerticalRollingTextView#setAdapterQuiet(VerticalRollingTextAdapter)}
     *
     * @param adapter 新的Adapter
     */
    public void setAdapter(VerticalRollingTextAdapter adapter) {

        if (null == adapter) {
            throw new RuntimeException("adapter不能为空");
        }

        if (null == mVerticalRollingTextAdapter || adapter != mVerticalRollingTextAdapter) {
            boolean run = isRunning;
            if (run)
                stop();
            mVerticalRollingTextAdapter = adapter;
            reset();
            if (run)
                run();
        }
     ...
     * @param adapter 新的Adapter
     */
    public void setAdapterQuiet(VerticalRollingTextAdapter adapter) {
    ...
    private class AnimListener extends AnimatorListenerAdapter {
        ...
        public void onAnimationEnd(Animator animation) {
            if (mTempAdapter == null) {
                ...
                //2.计算出正确的角标
                mFirstVisibleIndex = mFirstVisibleIndex < mVerticalRollingTextAdapter.getItemCount() ? mFirstVisibleIndex : mFirstVisibleIndex % mVerticalRollingTextAdapter.getItemCount();
                //3.位置复位
                mScrollY = 0;
            } else {
                mVerticalRollingTextAdapter = mTempAdapter;
                mTempAdapter = null;
                reset();
            }
            if (isRunning)
