...
import com.hzy.views.recycler.cell.EmptyCell;
import com.hzy.views.recycler.cell.ErrorCell;
import com.hzy.views.recycler.cell.LoadMoreCell;
import com.hzy.views.recycler.cell.LoadNoMoreCell;
import com.hzy.views.recycler.cell.LoadingCell;
...
public class SimpleAdapter extends BaseAdapter {
    ...
    public static final int LOAD_NO_MORE_TYPE = Integer.MAX_VALUE - 5;
    ...
    public SimpleAdapter() {
        ...
        mLoadNoMoreCell = new LoadNoMoreCell(null);
    }
        ...
        if (manager instanceof GridLayoutManager) {
            ...
            gridLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                ...
                public int getSpanSize(int position) {
                    ...
                    return (viewType == ERROR_TYPE || viewType == EMPTY_TYPE || viewType == LOADING_TYPE || viewType == LOAD_MORE_TYPE || viewType == LOAD_NO_MORE_TYPE) ? gridLayoutManager.getSpanCount() : 1;
                }
            });
        }
    ...
    public void onViewAttachedToWindow(RecyclerView.ViewHolder holder) {
        ...
        if (isStaggeredGridLayout(holder)) {
            if (viewType == ERROR_TYPE || viewType == EMPTY_TYPE || viewType == LOADING_TYPE || viewType == LOAD_MORE_TYPE || viewType == LOAD_NO_MORE_TYPE) {
    ...
    public void showLoadMore(View loadMoreView, int height) {
        ...
        //设置默认高度
        if (height == 0) {
            int defaultHeight = dp2px(loadMoreView.getContext(), LoadMoreCell.mDefaultHeight);
    ...
    public boolean isShowLoadNoMore() {
        return mIsShowLoadNoMore;
    }

    public void showLoadNoMore() {
        clear();
        mIsShowLoadNoMore = true;
        add(mLoadNoMoreCell);
    }

    public void showLoadNoMore(View loadNoMoreView) {
        showLoadNoMore(loadNoMoreView, 0);
    }

    public void showLoadNoMore(View loadNoMoreView, int viewHeight) {
        if (null == loadNoMoreView) {
            return;
        }
        clear();
        mIsShowLoadNoMore = true;
        mLoadNoMoreCell.setView(loadNoMoreView);
        mLoadNoMoreCell.setHeight(viewHeight);
        add(mLoadNoMoreCell);
    }
    ...
    private void checkNotContainSpecailCell() {
        ...
        if (mData.contains(mLoadNoMoreCell)) {
            mData.remove(mLoadNoMoreCell);
        }
    }
