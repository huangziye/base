package com.hzy.views.stepview;
...
/**
 * Created by ziye_huang on 2018/5/2.
 */
...
public class StepView extends View {
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.AT_MOST) {
            height = getPaddingTop() + getPaddingBottom() + (Math.max(selectedCircleRadius, doneCircleRadius)) * 2 + (displayMode == DISPLAY_MODE_WITH_TEXT ? textPadding : 0);
            if (!steps.isEmpty()) {
    ...
    private int[] getCirclePositionsWithText(int[] textWidth) {
        ...
        result[textWidth.length - 1] = getMeasuredWidth() - getPaddingRight() - Math.max(textWidth[textWidth.length - 1] / 2, selectedCircleRadius);
        if (result.length < 3) {
            return result;
        }
    ...
    private void drawStep(Canvas canvas, int step, int circleCenterX, int circleCenterY) {
        ...
        if (isSelected && !isDone) {
            ...
            int radius;
            if (state == ANIMATE_STEP_TRANSITION && (animationType == ANIMATION_CIRCLE || animationType == ANIMATION_ALL) && nextAnimatedStep < currentStep) {
            ...
            if (state == ANIMATE_STEP_TRANSITION && step == nextAnimatedStep && nextAnimatedStep > currentStep) {
                ...
                p.setColor(defaultCircleColor);
                canvas.drawCircle(circleCenterX, circleCenterY, selectedCircleRadius, p);
            }
        }
    }
    ...
    private void drawCheckMark(Canvas canvas, int circleCenterX, int circleCenterY) {
        ...
        Rect bounds = new Rect((int) (circleCenterX - width * 4.5), (int) (circleCenterY - width * 3.5), (int) (circleCenterX + width * 4.5), (int) (circleCenterY + width * 3.5));
        canvas.drawLine(bounds.left + 0.5f * width, bounds.bottom - 3.25f * width, bounds.left + 3.25f * width, bounds.bottom - 0.75f * width, paint);
        canvas.drawLine(bounds.left + 2.75f * width, bounds.bottom - 0.75f * width, bounds.right - 0.375f * width, bounds.top + 0.75f * width, paint);
    }
